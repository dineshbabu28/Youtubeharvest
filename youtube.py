    ...
    for i in range(0, len(v_ids), 50):
        ...
        for video in response['items']:
            ...
            video_details = dict(Channel_name=video['snippet']['channelTitle'],
                                ...
                                Duration=duration,  # Updated duration with parsed value in seconds
...
def insert_into_videos():
    ...
    for i in collections2.find({"Channel_name": user_inp}, {'_id': 0}):
        query = "INSERT INTO videos (Video_id,Views,Channel_id,Likes,Comments,Favorite_count,Channel_name,Title,Published_date,Caption_status,Duration) " \
                "VALUES ('{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}')".format(
            i.get('Video_id', ''), i.get('Views', ''),i.get('Channel_id', ''),i.get('Likes', ''),i.get('Comments', ''),i.get('Favorite_count', ''),i.get('Channel_name', '')
            ,i.get('Title', ''),i.get('Published_date', ''),i.get('Caption_status', ''),i.get('Duration', '')
        )
        try:
            mycursor.execute(query)
            mydb.commit()
...
def insert_into_comments():
    ...
    for vid in collections1.find({"Channel_name": user_inp}, {'_id': 0}):
        for i in collections2.find({'Video_id': vid.get('Video_id', '')}, {'_id': 0}):
            query2 = "INSERT INTO comments (Comment_id, Video_id, Comment_author, Comment_text, Comment_posted_date) " \
                     "VALUES ('{}', '{}', '{}', '{}', '{}')".format(
                i.get('Comment_id', ''), i.get('Video_id', ''), i.get('Comment_author', ''),
                i.get('Comment_text', ''), i.get('Comment_posted_date', '')
            )
            try:
